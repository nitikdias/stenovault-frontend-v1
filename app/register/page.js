...
export default function Register() {
  ...
  const stopRecording = async () => {
    try {
      ...
      const res = await fetch('http://127.0.0.1:8000/upload-User', {
        method: 'POST',
        body: formData,
      });
  ...
  const processRecording = async () => {
    try {
      const res = await fetch('http://127.0.0.1:8000/transcribeUser',{
      method:"GET",
      headers: {
  ...
  try {
    const res = await fetch('http://127.0.0.1:8000/register-speaker', {
      method: 'POST',
  ...
  if (!usersVisible) {
    try {
      const res = await fetch('http://127.0.0.1:8000/list-users',{
      method:"GET",
      headers: {
        "ngrok-skip-browser-warning": "true",  // ✅ correct spelling
        "Content-Type": "application/json"}
...
useEffect(() => {
    const clearBackend = async () => {
      try {
        const res = await fetch('http://127.0.0.1:8000/clear_live', {
          method: 'POST',
        });
